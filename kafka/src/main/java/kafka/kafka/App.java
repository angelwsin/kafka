...
public class App 
{
     ...
     * 
     * kafka的不足  1.批合并发送  消息丢失
     *           2.分布式事务不支持
     *           3.Kafka消费失败不支持重试
     *           4.Kafka支持消息顺序，但是一台Broker宕机后，就会产生消息乱序
     *           5.Kafka不支持定时消息
     *           
     * kafka的消费方使用拉的模式（Kafka使用短轮询方式，实时性取决于轮询间隔时间） 没有使用推的模式
     * 拉式系统的不足之处在于，如果broker没有数据，消费者可能会在紧密循环中结束轮询，有效地等待数据到达。 
     * 为了避免这种情况，我们在我们的拉取请求中有参数，允许消费者请求在“长轮询”中等待直到数据到达（并且可选地等待直到给定的字节数可用以确保大的传送大小）。
     * 
     * kafka处理不同。 我们的主题分为一组完全有序的分区，每个分区在每个订阅的消费者组中在任何给定的时间只有一个消费者消耗。 
     * 这意味着消费者在每个分区中的位置只是一个整数，下一个消息消耗的偏移量。 这使得关于消耗的状态非常小，每个分区只有一个数字。 这个状态可以定期检查点。 这使得等同于消息确认非常便宜。
     * 消费者可以故意地倒回到旧的偏移量并重新消耗数据。
     * 
     */
}
